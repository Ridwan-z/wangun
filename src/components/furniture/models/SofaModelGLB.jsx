...
// Target size (meters) from furniture catalog: 1.8 x 0.8 x 0.9
const TARGET_SIZE = { x: 1.8, y: 0.8, z: 0.9 };
...
class GLBErrorBoundary extends Component {
  ...
  componentDidCatch(error) {
    console.error(`Failed to load ${MODEL_PATH}, falling back to primitive sofa.`, error);
  }
...
function SofaGLBInner(props) {
  ...
  useEffect(() => {
    ...
    console.log('--- SofaModelGLB Debug ---');
...
export function SofaModelGLB(props) {
  return (
    <GLBErrorBoundary fallback={<SofaModel color={props.color} isSelected={props.isSelected} />}>
