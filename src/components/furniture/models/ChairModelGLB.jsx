...
// Target size (meters) from furniture catalog: 0.5 x 0.9 x 0.5
const TARGET_SIZE = { x: 0.5, y: 0.9, z: 0.5 };
...
class GLBErrorBoundary extends Component {
  ...
  componentDidCatch(error) {
    console.error(`Failed to load ${MODEL_PATH}, falling back to primitive chair.`, error);
  }
...
function ChairGLBInner(props) {
  ...
  useEffect(() => {
    ...
    console.log('--- ChairModelGLB Debug ---');
...
export function ChairModelGLB(props) {
  return (
    <GLBErrorBoundary fallback={<ChairModel color={props.color} isSelected={props.isSelected} />}>
